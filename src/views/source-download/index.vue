...
export default {
  ...
  methods: {
    ...
    initSpread(spread) {
      ...
      this.sheet_PC.options.clipBoardOptions =
        GC.Spread.Sheets.ClipboardPasteOptions.values;
      //渲染数据
      ...
      var filter_PC = new this.spreadNS.Filter.HideRowFilter(
        ...
        new this.spreadNS.Range(
          0,
          0,
          this.sheet_PC.getRowCount(),
          this.sheet_PC.getColumnCount()
        )
      );
        ...
        ContextMenu.prototype.onOpenMenu = (
          ...
          if (hitInfo.worksheetHitInfo) {
            ...
            itemsDataForShown.forEach((item, index) => {
              ...
              if (item.name === "gc.spread.unhideRows") {
                item.disable = false;
              }
              if (item.name === "gc.spread.hideColumns") {
                item.disable = false;
              }
              if (item.name === "gc.spread.unhideColumns") {
                item.disable = false;
              }
            });
          }
    ...
    setDateStyle(spread) {
      ...
      if (cellValue === "组套1") {
        ...
        if (
          ...
          // sheet.setStyle(0, 20, this.startDate);
          //   sheet.setStyle(i, 21, this.endDate);
          // }
          sheet
            .getRange(-1, 20, -1, 2)
            .cellButtons([
    ...
    setListStyle(spread) {
      ...
      sheet
        .getRange(-1, 27, -1, 3)
        .cellButtons(this.scheduleList.cellButtons)
        .dropDowns(this.scheduleList.dropDowns);
      sheet_PC
        .getRange(-1, 43, -1, 2)
        .cellButtons(this.scheduleList.cellButtons)
        .dropDowns(this.scheduleList.dropDowns);
    ...
    setColumnLock() {
      ...
      //设置前17列不可编辑
      this.sheet.getRange(-1, 0, -1, 17).locked(true);
      //设置第17列后可编辑
      this.sheet
        .getRange(-1, 18, -1, this.sheet.getColumnCount() - 18)
        .locked(false);
      //设置前29列不可编辑
      this.sheet_PC.getRange(-1, 0, -1, 29).locked(true);
      //设置29列后不可编辑
      this.sheet_PC
        .getRange(-1, 30, -1, this.sheet_PC.getColumnCount() - 29)
        .locked(false);
      ...
      this.sheet.options.protectionOptions = lockOption;
      this.sheet.options.isProtected = true;
      this.sheet_PC.options.protectionOptions = lockOption;
      this.sheet_PC.options.isProtected = true;
    },
    setNumberValidator() {
      let dv = new GC.Spread.Sheets.DataValidation.createNumberValidator(
      ...
      dv.inputTitle("tip");
      this.sheet.setDataValidator(-1, 23, dv);
    }
  }
};
</script>
